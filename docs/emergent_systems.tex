...
\usepackage{lmodern}  % scalable font; required for microtype's font expansion
\usepackage{microtype}  % full microtype: protrusion + expansion clears most Underfull/Overfull \hbox
...
\usepackage{enumitem}
\usepackage{array}  % required for >{\raggedright\arraybackslash}p{...} column specs
\usepackage{cite}
\usepackage{url}
\usepackage{tikz}
...
% Reduces benign Underfull \hbox warnings from IEEE's narrow two-column
% measure on draft text. Doesn't change final typography quality.
...
\begin{document}
\maketitle
% IEEE's narrow two-column measure produces residual Underfull \hbox
% warnings in paragraphs containing long unbreakable inline math, even
% with full microtype expansion. Suppress reports below moderate badness
% (5000) and allow variable column heights to clear the Underfull \vbox.
\sloppy
\hbadness=5000
\raggedbottom
...
\[
  \begin{aligned}
    ...
    \;&\iff\; k_1 \leq k_2 \\
    &\quad \text{and } \sigma_{1,i} \leq \sigma_{2,i} \;\forall i \leq k_1.
  \end{aligned}
\]
...
\scriptsize
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.13\textwidth} >{\raggedright\arraybackslash}p{0.205\textwidth} >{\raggedright\arraybackslash}p{0.19\textwidth} >{\raggedright\arraybackslash}p{0.19\textwidth} >{\raggedright\arraybackslash}p{0.19\textwidth}@{}}
\hline
...
\begin{itemize}[leftmargin=1.5em,itemsep=2pt]
  ...
  \item coupling $\kappa = (\kappa_{\mathrm{FL}\to\mathrm{LLM}},\,
        \kappa_{\mathrm{LLM}\to\mathrm{FL}})$, where
        $\kappa_{\mathrm{FL}\to\mathrm{LLM}}$ renders a text-prompt
        context from the Flow-Lenia frame and
        $\kappa_{\mathrm{LLM}\to\mathrm{FL}}$ supplies a per-creature
        scalar viability bonus indexed by the entity's support $S$ each
        tick.
\end{itemize}
...
Whether
\[
  \sup_{\Pi}\,\CE\bigl(M(\mathcal{S}),\Pi\bigr) > 0
\]
holds for this system is a concrete instance of L1 (= renamed OP1; see
\S\ref{sec:op1}).
...
\begin{enumerate}[leftmargin=2em,itemsep=2pt]
  ...
  \item Apply the viability filter $\mu_{t+1}=F(\mu')$ and set
    $x_{t+1}=x'$.
  \item Evaluate $(o_t,a_{t+1})=O_s((x,\mu)_{t-w:t+1},a_t)$,
    update any archive, and test the saturation criterion.
  \item Return trajectory summaries, observer outputs, archive
    state, and reproducibility metadata.
\end{enumerate}
...
vitality profile preserved: life-producing pairs at profile
$(k, \boldsymbol{\sigma})$ correspond to 4-tuple systems in the image
of $\mathrm{Decomp}$ at matching $(k, \boldsymbol{\sigma})$.
\end{namedconjecture}
