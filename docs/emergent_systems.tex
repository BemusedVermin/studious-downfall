...
example and implementation checklist. The paper also separates established
...
operator $\diamond_\rho$ of \S\ref{sec:variation-comp} extends this to
...
predicates the rest of the system relies on. The implementation checklist
of \S\ref{sec:checklist} therefore requires the viability formalism to be
...
(cf.\ \S\ref{sec:variation-comp}) when they are not. The mixture form
...
of the observer specification and is reported in the implementation
checklist (\S\ref{sec:checklist}).
...
of \S\ref{sec:variation-comp} with $O$'s archive state playing the role of
...
implementation checklist (\S\ref{sec:checklist}).

These are starting points only; an author who departs from them must
specify the alternative explicitly in the checklist of
\S\ref{sec:checklist}.
...
resulting system remains an instance of the same scaffold. Conformance
(\S\ref{sec:checklist}) requires only that each ablation reports its
choices in the same template.
...
synchronization boundaries, complexity assumptions, and reporting checklist
...
\topic{Implementation Checklist}\label{sec:checklist}

A reproducible $\mathcal{S}$-implementation reports, for each slot:
\begin{enumerate}[leftmargin=2em,itemsep=2pt]
...
A conformant implementation supplies at least the following control-flow
template, specialised to its chosen slots:
\begin{enumerate}[leftmargin=2em,itemsep=2pt]
...
Two implementations are \emph{conformant} with respect to the framework
when they (i) agree on the typing of each slot
(\S\ref{sec:framework}--\S\ref{sec:observer}); (ii) report all eleven items
of the checklist with values, not just slot names; (iii) preserve the
iteration order of item~8 or explicitly declare a variant; and (iv)
specify the entity-detection rule consistently with
Definition~\ref{def:entity}. Conformance does \emph{not} require choice of
the same fitness landscape, observer family, or coarse-graining $\Pi$;
those are user-level choices. It does require that a comparison across two
implementations be a comparison of choices, not of unstated defaults.
