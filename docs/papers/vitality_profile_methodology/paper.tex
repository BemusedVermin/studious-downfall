...
sufficient detail for an external researcher to implement and
validate it. The test fixtures live in
\texttt{vitality\_computation.md}:
%
\begin{itemize}[leftmargin=2em,itemsep=2pt]
  \item hurricane gives $(0, ())$;
  \item converged GA gives $(1, (0))$;
  \item active Tierra gives $(2, (1, 1))$;
  \item and so on.
\end{itemize}
...
\begin{enumerate}[leftmargin=2em,itemsep=4pt]
  ...
  \item \textbf{Implementation reference (UNPROVEN).} Describe the
    ...
    \texttt{FRAMING\_PIVOT\_TASKS.md} Part 2, task C-5). Show how
    it composes existing primitives from \texttt{emergence.py},
    \texttt{entity.py}, \texttt{viability.py}, and
    \texttt{observer.py}.
  \item \textbf{Empirical validation (UNPROVEN).} Run the algorithm
    on the regression-test fixtures. Verify that the verdicts match
    the structural predictions. Where they disagree, distinguish
    estimator error from algorithm error.
